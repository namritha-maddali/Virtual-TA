...
OUTPUT_JSON = "data/raw/discourse_meta_data.json"
OUTPUT_TXT = "data/raw/discourse_posts.txt"
...
def scrape_posts(playwright):
    ...
    while True:
        ...
        try:
            pre = page.locator("pre").text_content(timeout=5000)
            data = json.loads(pre)
        except:
            try:
    ...
    for topic in all_topics:
        ...
        print(f"{topic_url}")
        try:
            page.goto(topic_url, timeout=10000)
            pre = page.locator("pre").text_content(timeout=5000)
            topic_data = json.loads(pre)
        ...
        for post in posts:
            ...
            plain_text_output.append(f"{item['url']}\n{item['content']}\n\n")
    ...
    with open(OUTPUT_TXT, "w", encoding="utf-8") as f:
        f.writelines(plain_text_output)

    print(f"Scraped {len(filtered_posts)} posts from {DATE_FROM.date()} to {DATE_TO.date()}")
    browser.close()
